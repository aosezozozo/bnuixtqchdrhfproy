...
setup_nat64() {
    ...
    declare -a dns64_servers=(
        "Level66 Primary|2001:67c:2960::64"
...
EOF
    ...
    if [ -n "$test_result" ]; then
        ...
        echo -e "${yellow}! DNS64 解析未返回结果${re}"
    fi
...
remove_nat64() {
    ...
    [ -f /usr/local/bin/dns64-setup.sh ] && rm /usr/local/bin/dns64-setup.sh
...
while true; do
    ...
    if grep -q "2001:67c:2960::\|2001:67c:2b0::\|2a00:1098:2c::\|2a01:4f8:c2c:123f::" /etc/resolv.conf 2>/dev/null; then
    ...
    case $choice in
        ...
        2)
            del_ip6tables
            [ -f /etc/iptables/rules.v6 ] && rm /etc/iptables/rules.v6
